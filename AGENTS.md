...
- `OrderRecord` and `DowntimeRecord` have partial `idx_*_record_pending` indexes on `WorkplaceID` `where date_time_end IS NOT NULL AND deleted_at IS NULL AND note NOT ILIKE '%updated%'` for the mes_webservice job that stamps closed records with `;updated;` every minute. Without them that job scans both tables end to end. The query must repeat the predicate as a literal, word for word (mes_webservice `pendingRecordCondition`): PostgreSQL cannot match a partial index against a bound parameter. They are built with `option:CONCURRENTLY` so the build does not block writes, and system_service checks `pg_index.indisvalid` for them after the migration.

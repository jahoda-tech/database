...
type DowntimeRecord struct {
	...
	DateTimeEnd   sql.NullTime `gorm:"index"`
	WorkplaceID   int          `gorm:"uniqueIndex:unique_downtime_record;index;index:idx_downtime_record_open,where:date_time_end IS NULL;index:idx_downtime_record_pending,where:date_time_end IS NOT NULL AND note NOT ILIKE '%updated%'"`
	Workplace     Workplace
...
type OrderRecord struct {
	...
	Operation          Operation
	WorkplaceID        int `gorm:"uniqueIndex:unique_order_record;index:idx_order_datetime_workplace;index:idx_order_workplace;index:idx_order_record_open,where:date_time_end IS NULL;index:idx_order_record_pending,where:date_time_end IS NOT NULL AND note NOT ILIKE '%updated%'"`
	Workplace          Workplace

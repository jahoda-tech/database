...
- `StateRecord` has `idx_state_record_workplace_time` on `(workplace_id, date_time_start)` for "latest state per workplace" lookups (state_service on every state change, mes_webservice on every 60 s poll). The unique index cannot serve them because `state_id` sits between the two columns. It is built with `option:CONCURRENTLY` for the same reason as `idx_digital_record_zero`, so check `pg_index.indisvalid` after the first migration.
- `OrderRecord` and `DowntimeRecord` have partial `idx_*_record_pending` indexes `where date_time_end IS NOT NULL AND note NOT ILIKE '%updated%'` for the mes_webservice job that stamps closed records every minute. The query must repeat that predicate as a literal, word for word: PostgreSQL cannot match a partial index against a bound parameter.
- `User.Email` and `State.Type` carry standalone indexes: `unique_user` leads with `first_name`, so `where email = ?` (run by mes_webservice on every request) cannot use it.
- `DevicePortDigitalRecord` has a partial `idx_digital_record_zero` on `(device_port_id, date_time) where data = 0` so counter COUNTs (state_service order counts) run as index-only scans instead of visiting a heap page per row. It is built with `option:CONCURRENTLY` because a plain build blocks inserts on this large table. Index-only scans skip the heap only on vacuumed pages, so set per database: `ALTER TABLE device_port_digital_records SET (autovacuum_vacuum_insert_scale_factor = 0, autovacuum_vacuum_insert_threshold = 100000)`. An interrupted concurrent build leaves an INVALID index that AutoMigrate never rebuilds, so check `pg_index.indisvalid`.
